...
---
### ♠️ 2. BlackJack Game  
● Blackjack — A fully playable multiplayer Blackjack game built in C++ with no external dependencies, just the standard
  library. The game runs in the console and supports any number of players against a single dealer, with a play-again
  loop so you can run round after round.
                                                                                                                          The codebase is organized into six classes, each in its own header and source pair: Card, Deck, Hand, Player,
  BlackjackGame, and a thin main entry point. The deck shuffles using std::mt19937 seeded from std::random_device and     automatically reshuffles itself if it runs out mid-round. Ace handling is fully automatic — treated as 11 when it
  helps, 1 when 11 would bust — and the dealer follows standard casino rules, hitting below 17 and standing otherwise.
...
---
### 🔢 4. ASCII Optimization (Huffman Trees)  
...
- **LinkedIn:** https://www.linkedin.com/in/josh-wiersema-526452377/
---
